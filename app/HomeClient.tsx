...
export default function HomeClient({ initialSettings }: { initialSettings: any }) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF9] font-sans text-[#3A2D23] overflow-x-hidden selection:bg-[#B3744A] selection:text-white">
      ...
      {/* Popup แจ้งเตือนการใช้งานข้อมูลเพื่อการวิจัย */}
      <ResearchDisclaimerModal />

      {/* Popup ประกาศข่าวสารของเว็บไซต์ */}
      <AnnouncementModal
        enabled={initialSettings?.announcementEnabled === true}
        title={initialSettings?.announcementTitle}
        body={initialSettings?.announcementBody}
      />
    </div>
  );
}
